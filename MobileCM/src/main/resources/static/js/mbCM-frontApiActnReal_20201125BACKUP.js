...
function lalngCll(resultCode,resultCount,resultMessage) {
...
function mbCMmapCll(rem_dt,com_scd,sti_cd,sti_nm,orm_qty,orm_amt, constcst_sum) {//스케줄 불러오기
     var dateapi = $('#date2').val();//현재 조정한 날짜
	$.ajax({
...
function tmserp_allocation(rem_dt,com_scd,sti_cd,sti_nm,orm_qty,orm_amt, constcst_sum) {//분배결과리스트
     var pop = $('.pop_assgnStrtBx');
     pop.removeClass('opn');
     $('.alrtPop').addClass('opn');
