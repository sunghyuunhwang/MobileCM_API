...
function chngTmDon(resultCode,resultCount,resultMessage) {//팀 변경 완료
	var $selected_from = $("#assgnSub_lst li.on");
	if($selected_from.size() == 0){
		alert("변경 대상 건을 선택하세요!");
		return false;
	}
	var from_com_ssec = $selected_from.find('input[name=from_com_ssec]').val();
	var from_rem_dt = $selected_from.find('input[name=from_rem_dt]').val();
	var from_rem_seq = $selected_from.find('input[name=from_rem_seq]').val();
	var from_plm_no = $selected_from.find('input[name=from_plm_no]').val();
	var from_orm_no = $selected_from.find('input[name=from_orm_no]').val();
	
	console.log(from_com_ssec);
	console.log(from_rem_dt);
	console.log(from_rem_seq);
	console.log(from_plm_no);
	console.log(from_orm_no);

	var $selected_to = $("#chngTmLst li.on");
	if($selected_to.size() == 0){
		alert("변경 할 팀을 선택하세요!");
		return false;
	}
	...
	console.log(from_com_scd);
	console.log(from_com_ctsec);	
	console.log(from_sti_cd);
