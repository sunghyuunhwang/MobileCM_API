...
//              k_sti_cd: "YA142",
	    ...
	    success: function(response){
         ...
         //},
	    error: function (request, status, error){
              console.log("resultCode:"+request.resultCode+"\n"+"resultCount:"+request.resultCount+"message:"+request.resultMessage+"\n"+"error:"+error);
              alert('데이터를 불러올수 없습니다.');
	    }
	  });
		    ...
		    $.each(list, function(idx, response) {
			    ...
			    var assgn_lst = "<li>";
			        assgn_lst += "<dl class='dfDl assgnBx nrrwwid _nrrw'>";
    			        ...
    			        assgn_lst += "<span class='txt'>"+response.sti_nm+"</span><input type='checkbox' class='chck' id='chks_"+idx+"'/><label for='chks_"+idx+"'></label>";
    			        assgn_lst += "</dt>";
    			        assgn_lst += "<dd>";
		    ...
		    $.each(list, function(idx, response) {
			    ...
			    var assgnPop_lst = "<li>";
			        assgnPop_lst += "<dl class='dfDl assgnBx'>";
    			        assgnPop_lst += "<dt class='fao'><span class='crlclOn'></span>"+response.sti_nm+"<input type='checkbox' class='chck' name='vehicles'/></dt>";
    			        assgnPop_lst += "<dd>";
...
function tmsrpAllcStrt(resultCode,resultCount,resultMessage,mappingKey) {//분배하기
     var dateapi = $('#date2').val();//현재 조정한 날짜
     $.ajax({
	    url: "/v1/api/erp/tmserp_Allocation",
	    type: "GET",
	    cache: false,
	    dataType: "json",
         data: {
		    ...
		    $.each(list, function(idx, response) {
			    ...
			    var assgn_lst = "<li onclick='assgnsubInf()'>";
			        assgn_lst += "<span class='crlclOn'></span>";
    			        ...
    			        assgn_lst += "<span class='txt'>"+response.orm_nm+"</span>";
	...
	$.ajax({
	    ...
	    success: function(list){
		    $.each(list, function(idx, response) {
				...
				$('#chsAssgnInfBx').append('<script>chngTmLst();chngTmCls();</script>');
			 console.log(response);
 	          });
	    },
         //complete:function{
...
function assgninfLstdt(rem_dt,com_scd,sti_cd,sti_nm,orm_qty,orm_amt, constcst_sum) {//시공품목리스트
